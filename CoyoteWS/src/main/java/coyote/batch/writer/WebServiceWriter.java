...
import coyote.batch.web.InvocationException;
...
import coyote.batch.web.auth.AuthenticationException;
import coyote.batch.web.auth.Authenticator;
import coyote.batch.web.auth.NullAuthenticator;
...
public class WebServiceWriter extends AbstractConfigurableComponent implements FrameWriter, ConfigurableComponent {
  ...
  public void open( TransformContext context ) {
    ...
    if ( resource == null ) {
      ...
      if ( resource == null ) {
        ...
        Log.debug( LogMsg.createMsg( Batch.MSG, "Writer.using_target", this.getClass().getSimpleName(), targetUrl ) );
        ...
        for ( DataField field : getConfiguration().getFields() ) {
          if ( ( field.getName() != null ) && field.getName().equalsIgnoreCase( BatchWS.AUTHENTICATOR ) ) {
            if ( field.isFrame() ) {
              DataFrame cfg = (DataFrame)field.getObjectValue();
              try {
                authenticator = BatchWS.configAuthenticator( cfg );
                break;
        ...
        for ( DataField field : getConfiguration().getFields() ) {
          if ( ( field.getName() != null ) && field.getName().equalsIgnoreCase( BatchWS.PROXY ) ) {
            if ( field.isFrame() ) {
              try {
                proxy = BatchWS.configProxy( (DataFrame)field.getObjectValue() );
              } catch ( ConfigurationException e ) {
                Log.fatal( e );
        ...
        for ( DataField field : getConfiguration().getFields() ) {
          if ( ( field.getName() != null ) && field.getName().equalsIgnoreCase( BatchWS.PROTOCOL ) ) {
            if ( field.isFrame() ) {
              try {
                parameters = BatchWS.configParameters( (DataFrame)field.getObjectValue() );
              } catch ( ConfigurationException e ) {
                Log.fatal( e );
        ...
        try {
          ...
          for ( DataField field : getConfiguration().getFields() ) {
            if ( field.getName() != null && field.getName().equalsIgnoreCase( BatchWS.DECORATOR ) ) {
              if ( field.isFrame() ) {
                ...
                for ( DataField cfgfield : cfgFrame.getFields() ) {
                  if ( cfgfield.isFrame() ) {
                    if ( StringUtil.isNotBlank( cfgfield.getName() ) ) {
                      BatchWS.configDecorator( cfgfield.getName(), (DataFrame)cfgfield.getObjectValue(), resource );
                    } else {
                      Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.configuration_must_be_named" ) );
                    }
                  } else {
                    Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.invalid_decorator_configuration_section" ) );
                  }
                }
              } else {
                Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.invalid_decorator_configuration_section" ) );
    ...
    Log.debug( LogMsg.createMsg( BatchWS.MSG, "Writer.init_complete", resource ) );
  }
  ...
  public void write( DataFrame frame ) {
    ...
    if ( expression != null ) {
      ...
      try {
        ...
        Log.warn( LogMsg.createMsg( BatchWS.MSG, "Writer.boolean_evaluation_error", e.getMessage() ) );
      }
    } else {
  ...
  public void close() throws IOException {
    ...
    Log.debug( LogMsg.createMsg( BatchWS.MSG, "Writer.records_processed", rowCounter, ( context != null ) ? context.getRow() : 0 ) );

    if ( resource != null ) {
      try {
        resource.close();
      } catch ( Exception e ) {
        Log.warn( LogMsg.createMsg( BatchWS.MSG, "Writer.close_error", e.getLocalizedMessage() ) );
      }
    }
