...
import coyote.batch.web.ExchangeType;
...
import coyote.batch.web.auth.AbstractAuthenticator;
import coyote.batch.web.auth.Authenticator;
import coyote.batch.web.decorator.AbstractDecorator;
...
public class BatchWS {
  ...
  public static final String NAME = "BatchWS";
  ...
  public static void configDecorator( String className, DataFrame cfg, Resource resource ) {
    Log.debug( LogMsg.createMsg( BatchWS.MSG, "Decorator.configuring_decorator", className, cfg ) );
    ...
    if ( className != null && StringUtil.countOccurrencesOf( className, "." ) < 1 ) {
      ...
      Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.no_decorator_class", cfg.toString() ) );
    }
      ...
      if ( object instanceof RequestDecorator ) {
        if ( cfg != null ) {
          try {
            ...
            Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.configuration_error", object.getClass().getName(), e.getClass().getSimpleName(), e.getMessage() ) );
          }
        }

        resource.addRequestDecorator( (RequestDecorator)object );
        Log.debug( LogMsg.createMsg( BatchWS.MSG, "Decorator.created_decorator", object.getClass().getName() ) );
      } else {
        Log.warn( LogMsg.createMsg( BatchWS.MSG, "Decorator.class_is_not_decorator", className ) );
      }
    } catch ( ClassNotFoundException | NoSuchMethodException | SecurityException | InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e ) {
      Log.error( LogMsg.createMsg( BatchWS.MSG, "Decorator.could_not_instantiate", className, e.getClass().getName(), e.getMessage() ) );
    }
   ...
   * 
   * @param cfg the configuration frame (must containd a class attribute)
   * 
  ...
  public static Authenticator configAuthenticator( DataFrame cfg ) throws ConfigurationException {
    ...
    try {
      ...
      if ( object instanceof Authenticator ) {
        ...
        Log.warn( LogMsg.createMsg( Batch.MSG, "BatchWS.instance_not_authenticator", className ) );
      }
