...
public class WebServiceReader extends AbstractFrameReader implements FrameReader, ConfigurableComponent {
  ...
  public void open( TransformContext context ) {
    ...
    for ( DataField field : getConfiguration().getFields() ) {
      if ( ( field.getName() != null ) && field.getName().equalsIgnoreCase( BatchWS.PROTOCOL ) ) {
        if ( field.isFrame() ) {
          try {
            protocol = BatchWS.configParameters( (DataFrame)field.getObjectValue() );
          } catch ( ConfigurationException e ) {
            Log.fatal( e );
    ...
    for ( DataField field : getConfiguration().getFields() ) {
      if ( ( field.getName() != null ) && field.getName().equalsIgnoreCase( BatchWS.BODY ) ) {
        if ( field.isFrame() ) {
          DataFrame cfg = (DataFrame)field.getObjectValue();
    ...
    selectorPattern = getConfiguration().getAsString( BatchWS.SELECTOR );
  ...
  private List<DataFrame> retrieveData() {
    ...
    retval.add( BatchWS.flatten( result ) );
