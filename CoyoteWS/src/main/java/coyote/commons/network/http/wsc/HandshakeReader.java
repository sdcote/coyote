...
class HandshakeReader {
  ...
  public HandshakeReader(final WebSocket websocket) {
    webSocket = websocket;
  }
  ...
  private void parseHttpHeader(final Map<String, List<String>> headers, final String header) {
    // Split 'header' to name & value.
    final String[] pair = header.split(":", 2);

    if (pair.length < 2) {
      // Ignore this header, it's missing something
      return;
    }
   ...
   */
  private byte[] readBody(final Map<String, List<String>> headers, final WebSocketInputStream input) {
    // Get the value of "Content-Length" header.
    final int length = getContentLength(headers);
  ...
  public Map<String, List<String>> readHandshake(final WebSocketInputStream input, final String key) throws WebSocketException {
    // Read the status line.
    final StatusLine statusLine = readStatusLine(input);

    // Read HTTP headers.
    final Map<String, List<String>> headers = readHttpHeaders(input);
  ...
  private Map<String, List<String>> readHttpHeaders(final WebSocketInputStream input) throws WebSocketException {
    // Create a map of HTTP headers. Keys are case-insensitive.
    final Map<String, List<String>> headers = new TreeMap<String, List<String>>(String.CASE_INSENSITIVE_ORDER);
   ...
   */
  private StatusLine readStatusLine(final WebSocketInputStream input) throws WebSocketException {
    String line;
   ...
   * </blockquote>
   */
  private void validateAccept(final StatusLine statusLine, final Map<String, List<String>> headers, final String key) throws WebSocketException {
    // Get the values of Sec-WebSocket-Accept.
    final List<String> values = headers.get("Sec-WebSocket-Accept");
    ...
    final String actual = values.get(0);

    // Concatenate.
    final String input = key + ACCEPT_MAGIC;
    ...
    try {
      ...
      final byte[] digest = md.digest(WebSocketUtil.getBytesUTF8(input));
   ...
   * </blockquote>
   */
  private void validateConnection(final StatusLine statusLine, final Map<String, List<String>> headers) throws WebSocketException {
    // Get the values of Upgrade.
    final List<String> values = headers.get("Connection");
    ...
    for (final String value : values) {
      ...
      final String[] elements = value.split("\\s*,\\s*");
  ...
  private void validateExtensionCombination(final StatusLine statusLine, final Map<String, List<String>> headers, final List<WebSocketExtension> extensions) throws WebSocketException {
    ...
    for (final WebSocketExtension extension : extensions) {
      ...
      final String message = String.format("'%s' extension and '%s' extension conflict with each other.", pmce.getName(), extension.getName());
   ...
   * </blockquote>
   */
  private void validateExtensions(final StatusLine statusLine, final Map<String, List<String>> headers) throws WebSocketException {
    // Get the values of Sec-WebSocket-Extensions.
    final List<String> values = headers.get("Sec-WebSocket-Extensions");
    ...
    final List<WebSocketExtension> extensions = new ArrayList<WebSocketExtension>();
    ...
    for (final String value : values) {
      ...
      for (final String element : elements) {
        ...
        final WebSocketExtension extension = WebSocketExtension.parse(element);
        ...
        if (webSocket.getHandshakeBuilder().containsExtension(name) == false) {
    ...
    // Agreed extensions.
    webSocket.setAgreedExtensions(extensions);
  }
   ...
   * </blockquote>
   */
  private void validateProtocol(final StatusLine statusLine, final Map<String, List<String>> headers) throws WebSocketException {
    // Get the values of Sec-WebSocket-Protocol.
    final List<String> values = headers.get("Sec-WebSocket-Protocol");
    ...
    // Protocol
    final String protocol = values.get(0);
    ...
    // from this client.
    if (webSocket.getHandshakeBuilder().containsProtocol(protocol) == false) {
    ...
    // Agreed protocol.
    webSocket.setAgreedProtocol(protocol);
  }
   ...
   */
  private void validateStatusLine(final StatusLine statusLine, final Map<String, List<String>> headers, final WebSocketInputStream input) throws WebSocketException {
    ...
    // Read the response body.
    final byte[] body = readBody(headers, input);
   ...
   * </blockquote>
   */
  private void validateUpgrade(final StatusLine statusLine, final Map<String, List<String>> headers) throws WebSocketException {
    // Get the values of Upgrade.
    final List<String> values = headers.get("Upgrade");
