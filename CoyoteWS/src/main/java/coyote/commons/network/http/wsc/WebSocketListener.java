 ...
 *
 * <p>An implementation of this interface should be added by {@link
 * WebSocket#addListener(WebSocketListener)} to a {@link WebSocket}
 * instance before calling {@link WebSocket#connect()}.
 *
 * <p>{@link WebSocketAdapter} is an empty implementation of this interface.
 *
...
public interface WebSocketListener {
  ...
  /**
   * Called when an {@code on<i>Xxx</i>()} method threw a {@code Throwable}.
   *
   * @param websocket The WebSocket.
   *
   * @param cause The {@code Throwable} an {@code on<i>Xxx</i>} method threw.
   *
   ...
   *
   * @throws Exception An exception thrown by an implementation of this method.
   *         The exception is passed to {@link #handleCallbackError(WebSocket, 
   *         Throwable)}.
   */
   ...
   * HTTP header names such as {@code "Sec-WebSocket-Accept"}. Note that the 
   * comparator used by the map is {@link String#CASE_INSENSITIVE_ORDER}.
   *
   * @throws Exception An exception thrown by an implementation of this method. 
   *         The exception is passed to {@link #handleCallbackError(WebSocket, 
   *         Throwable)}.
   ...
   *
   * <p>Note that this method is called only when {@code 
   * connectAsynchronously()} was used and the {@link WebSocket#connect() 
   * connect()} executed in the background thread failed. Neither direct 
   * synchronous {@code connect()} nor 
   ...
   * @param websocket The WebSocket.
   *
   * @param serverCloseFrame The 
   *        <a href="https://tools.ietf.org/html/rfc6455#section-5.5.1">close 
   *        frame</a> which the server sent to this client. This may be {@code
   *         null}.
   *
   * @param clientCloseFrame The 
   *        <a href="https://tools.ietf.org/html/rfc6455#section-5.5.1">close 
   *        frame</a> which this client sent to the server. This may be {@code 
   *        null}.
   *
   * @param closedByServer {@code true} if the closing handshake was started 
   *        by the server. {@code false} if the closing handshake was started 
   *        by the client.
   *
   * @throws Exception An exception thrown by an implementation of this method.
   *         The exception is passed to {@link #handleCallbackError(WebSocket, 
  ...
  /**
   * Call when an error occurred. This method is called before an {@code 
   * on<i>Xxx</i>Error} method is called.
   *
   * @param websocket The WebSocket.
   *
  ...
  /**
   * Called when a frame was received. This method is called before  an {@code 
   * on<i>Xxx</i>Frame} method is called.
   *
   * @param websocket The WebSocket.
   *
   ...
   *
   * <p>Some WebSocket server implementations close a WebSocket connection 
   * without sending a <a href="https://tools.ietf.org/html/rfc6455#section-5.5.1">close 
   * frame</a> to a client in some cases. Strictly speaking, this behavior is 
   * a violation against the specification 
   * (<a href="https://tools.ietf.org/html/rfc6455">RFC 6455</a>). However, 
   * this library has allowed the behavior by default since the early 
   * releases. Even if the end of the input stream of a WebSocket connection 
   * were reached without a close frame being received, it would trigger 
   * neither {@link #onError(WebSocket, WebSocketException) onError()} method 
   * nor {@link #onFrameError(WebSocket, WebSocketException, WebSocketFrame) 
   * onFrameError()} method. If you want to makethis library report an error 
   * in the case, pass {@code false} to {@link 
   * WebSocket#setMissingCloseFrameAllowed(boolean)} method.
   *
   * @param websocket The WebSocket.
   *
   * @param cause An exception that represents the error. When the error 
   *        occurred because of {@link java.io.InterruptedIOException 
   *        InterruptedIOException}, {@code exception.getError()} returns 
   *        {@link WebSocketError#INTERRUPTED_IN_READING}. For other IO 
   *        errors, {@code exception.getError()} returns {@link 
   *        WebSocketError#IO_ERROR_IN_READING}. Other error codes denote 
   *        protocol errors, which imply that some bugs may exist in either or 
   *        both of the client-side and the server-side implementations.
   *
   * @param frame The WebSocket frame. If this is not {@code null}, it means 
   *        that verification of the frame failed.
   *
   * @throws Exception An exception thrown by an implementation of this method.
   *         The exception is passed to {@link #handleCallbackError(WebSocket, 
   *         Throwable)}.
  ...
  /**
   * Called when a WebSocket frame was sent to the server, but not yet flushed.
   *
   * @param websocket The WebSocket.
   *
  ...
  /**
   * Called when a WebSocket frame was not sent to the server because a 
   * <a href="https://tools.ietf.org/html/rfc6455#section-5.5.1">close 
   * frame</a> has already been sent.
   *
   * <p> Note that {@code onFrameUnsent} is not called when {@link
  ...
  /**
   * Called when it failed to concatenate payloads of multiple frames to 
   * construct a message. The reason of the failure is probably out-of-memory.
   *
   * @param websocket The WebSocket.
   *
   * @param cause An exception that represents the error.
   *
   * @param frames The list of frames that form a message. The first element is 
   *        either a text frame and a binary frame, and the other frames are 
   *        continuation frames.
   *
   * @throws Exception An exception thrown by an implementation of this method.
   *         The exception is passed to {@link #handleCallbackError(WebSocket, 
  ...
  /**
   * Called when an error occurred when a frame was tried to be sent to the 
   * server.
   *
   * @param websocket The WebSocket.
   *
   ...
   * 
   * <p>The reason of the failure is probably out-of-memory.
   *
   * @param websocket The WebSocket.
   *
   * @param cause An exception that represents the error.
  ...
  /**
   * Called between after a thread is created and before the thread's {@code 
  ...
  /**
   * Called when an uncaught throwable was detected in either the reading 
   * thread (which reads frames from the server) or the writing thread (which 
