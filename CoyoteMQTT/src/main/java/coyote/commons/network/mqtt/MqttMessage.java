 ...
 * 
 * <p>The message includes a "payload" (the body of the message) represented as 
 * a byte array.</p>
 */
public class MqttMessage {
   ...
   */
  public static void validateQos( final int qos ) {
    if ( ( qos < 0 ) || ( qos > 2 ) ) {
   ...
   * 
   * <p>This is only applicable to messages received from the server.</p>
   * 
   ...
   * 
   * <p>This will only be set on messages received from the server.</p>
   * 
   * @return {@code true} if the message might be a duplicate.
   */
   ...
   * 
   * <p>For messages received from the server, this method returns whether or 
   * not the message was from a current publisher, or was "retained" by the 
   * server as the last message published on the topic.</p>
   *
   * @return {@code true} if the message should be, or was, retained by the 
   *         server.
   * 
   ...
   * 
   * <p>Has no effect when publishing messages.</p>
   * 
   * @param messageId
   */
   ...
   * changed.
   * 
   * @param mutable {@code true} if the values can be changed, {@code false} to 
   ...
   * most once (zero or one times). The message will not be persisted to disk,
   ...
   * if the server cannot process the message (for example, there is an 
   * authorization problem), then an {@link 
   * ClientListener#deliveryComplete(MqttDeliveryToken)}. Also known as "fire 
   * and forget".</li>
   *
   * <li>Quality of Service 1 - indicates that a message should be delivered at 
   * least once (one or more times). The message can only be delivered safely 
   * if it can be persisted, so the application must supply a means of 
   * persistence using {@code MqttConnectOptions}. If a persistence mechanism 
   * is not specified, the message will not be delivered in the event of a 
   * client failure. The message will be acknowledged across the network. This 
   * is the default QoS.</li>
   *
   ...
   * two-phase acknowledgment across the network. The message can only be 
   * delivered safely if it can be persisted, so the application must supply a 
   * means of persistence using {@code ConnectOptions}. If a persistence 
   * mechanism is not specified, the message will not be delivered in the event 
   * of a client failure.</li></ul>
   *
   * <p>If persistence is not configured, QoS 1 and 2 messages will still be 
   ...
   * not be maintained as client-side state will be lost.</p>
   *
   ...
   * engine.
   * 
   * <p>Sending a message with retained set to {@code true} and with an empty 
   * byte array as the payload e.g. {@code new byte[0]} will clear the retained 
   * message from the server. The default value is {@code false}.</p>
   *
   ...
   * 
   * <p>Makes an attempt to return the payload as a string. As the MQTT client 
   * has no control over the content of the payload it may fail.</p>
   * 
