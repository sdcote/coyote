 ...
 * 
 * <p>Range and bearing from radar to target. Course and speed of target. One 
 * message per target. Example:
 * <pre>$RATTM,11,11.4,13.6,T,7.0,20.0,T,0.0,0.0,N,,Q,,154125.82,A,*17</pre>
 */
public interface TTMSentence extends TimeSentence {
   ...
   * 
   * <p>A target is first in state QUERY while the radar works out firm data of 
   * the target. At first the calculated course and speed are rough and varies 
   * a lot; with time they settle at which point the target becomes TRACKING. 
   * A target no longer detected becomes LOST before TTM sentences will not be 
   * sent at all for the target.
   *
   ...
   * 
   * <p>AUTO indicates that the radar initiated the target. MANUAL indicates 
   * that the radar operator initiated the target in the radar. REPORTED 
   * indicates that the target came from another source.
   *
   ...
   * A target may be used to calculate own ship position.
   *
   * @param isReference True if this target is used to calculate own ship position.
   */
