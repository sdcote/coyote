...
public class FlatFileReader extends AbstractFrameReader implements FrameReader, ConfigurableComponent {
  ...
  public void open( TransformContext context ) {
    ...
    Log.debug( LogMsg.createMsg( Batch.MSG, "Reader.using_source_uri", source ) );
    ...
    if ( StringUtil.isNotBlank( source ) ) {
      ...
      if ( uri != null ) {
        ...
        if ( sourceFile != null ) {
          Log.debug( LogMsg.createMsg( Batch.MSG, "Reader.using_source_file", sourceFile.getAbsolutePath() ) );
        } else {
          ...
          if ( sourceFile != null ) {
            ...
            String msg = LogMsg.createMsg( Batch.MSG, "Reader.source_uri_not_file", getClass().getName(), source ).toString();
            Log.error( msg );
      ...
      if ( sourceFile.exists() && sourceFile.canRead() ) {
        ...
        String msg = LogMsg.createMsg( Batch.MSG, "Reader.could_not_read_from_source", getClass().getName(), sourceFile.getAbsolutePath() ).toString();
        Log.error( msg );
        context.setError( msg );
      }
    } else {
      String msg = LogMsg.createMsg( Batch.MSG, "Reader.no_source_specified", getClass().getName() ).toString();
      Log.error( msg );
      context.setError( msg );
    }
    ...
    if ( fieldcfg != null ) {
      ...
      Log.debug( LogMsg.createMsg( Batch.MSG, "Reader.configured_field_definitions", fields.size() ) );
      ...
      if ( selectorcfg != null ) {
        ...
        Log.debug( LogMsg.createMsg( Batch.MSG, "Reader.line_map_configured" ) );
      } else {
  ...
  class Parser {
    ...
    public DataFrame parse( String line ) {
      ...
      if ( DEFAULT != null ) {
        ...
        if ( format == null ) {
          Log.warn( LogMsg.createMsg( Batch.MSG, "Reader.could_not_find_format_for_key", key, context.getRow() + 1, keyStart, keyLength ) );
        } else {
          if ( Log.isLogging( Log.DEBUG_EVENTS ) )
            Log.debug( LogMsg.createMsg( Batch.MSG, "Reader.parsing_record_type", key, context.getRow() + 1 ) );
        }
      }
