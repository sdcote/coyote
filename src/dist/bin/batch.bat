...
if exist "%HOMEDRIVE%%HOMEPATH%\batch_pre.bat" call "%HOMEDRIVE%%HOMEPATH%\batch_pre.bat"
...
if exist "%USERPROFILE%\batch_pre.bat" call "%USERPROFILE%\batch_pre.bat"
...
@rem Add default JVM options here. You can also use JAVA_OPTS and BATCH_OPTS to
...
@rem Execute Coyote Batch
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %BATCH_OPTS% -Dapp.home="%APP_HOME%" -Dapp.work="%APP_WORK%" -classpath "%CLASSPATH%" coyote.loader.BootStrap %CMD_LINE_ARGS%
...
@rem - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
:fail
@rem Set variable BATCH_EXIT_CONSOLE if you need the _script_ return code 
@rem instead of the _cmd.exe /c_ return code!
@rem   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
if  not "" == "%BATCH_EXIT_CONSOLE%" exit 1
exit /b 1
...
if exist "%HOME%\batch_post.bat" call "%HOME%\batch_post.bat"

@rem - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
:homeDrivePathPost
if "%HOMEDRIVE%%HOMEPATH%"=="" goto userProfilePost
if "%HOMEDRIVE%%HOMEPATH%"=="%HOME%" goto userProfilePost
if exist "%HOMEDRIVE%%HOMEPATH%\batch_post.bat" call "%HOMEDRIVE%%HOMEPATH%\batch_post.bat"
...
if exist "%USERPROFILE%\batch_post.bat" call "%USERPROFILE%\batch_post.bat"
